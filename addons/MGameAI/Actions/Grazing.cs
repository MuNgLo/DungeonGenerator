...
[GlobalClass]
public partial class Grazing : ActionBaseClass, IAction
{
    [Export] private ulong grazeTime = 5000; // Time it takes to graze
    [Export] private float maxDistance = 80.0f; // At this distance and further, all things lose appeal.
    [Export] private int maxDistancePenalty = 50;
    [Export] private float reach = .5f; // How close we have to be to graze it
    [Export] private int targetThingID = -1;
    [Export] private Vector3 targetLocation = Vector3.Zero;

    private bool isGrazing = false;
    private ulong startedGrazing = 0;
    ...
    {
        if (debug) { Log($"Grazing::RegisterType() on {Unit.Name} for Type({typeName}) from {source}"); }
        typesOfThings[typeName] = source;
        if (!typesOfThings.Keys.Contains(typeName))
        {
            Log($"Grazing::RegisterType() Failed");
        }
    }
    ...
    public AISTATE Begin(AISTATE aiState, NavigationAgent3D navAgent)
    {
        ...
        if (debug) { Log($"Grazing::Begin() {Unit.Body.Name} started grazing a thing[{targetThingID}]"); }
    ...
    public AISTATE Continue(AISTATE aiState, NavigationAgent3D navAgent)
    {
        ...
        if (Unit.Awareness.GetClosest(visibleThings, out ViewObject thing))
        {
            ...
            {
                targetThingID = thing.aIObjectID;
        ...
        switch (Unit.MovementState)
        {
            ...
            default:
                if (Unit.GlobalPosition.DistanceTo(targetLocation) <= reach)
                {
                    if (!isGrazing) {
                        isGrazing = true;
                        startedGrazing = Time.GetTicksMsec();
                        break; 
                    }
                    if(isGrazing && startedGrazing + grazeTime < Time.GetTicksMsec())
                    {
                        Graze();
                        return AISTATE.RESET;
                    }
    ...
    private void Graze()
    {
        ...
        {
            if (debug) { Log($"Grazing::Graze({targetThingID}) Eat the Thing! {thing.Name}"); }
            if(thing is IVegitation)
            {
                Unit.GetParent<IPersonality>().Feed((thing as IVegitation).Consume());
            }
        }
    }
    ...
    private bool KnowAboutThings()
    {
        ...
        message = $"Dont know about any grazable. ({typesOfThings.Count}) {String.Join(',', typesOfThings.Keys)}.";
        return false;
    }
    ...
    {
        //if (debug) { GD.Print($"Grazing::MoveAddToVisible() {aIObjectID}"); }
    ...
    {
        //if (debug) { GD.Print($"Grazing::MoveAddToNotVisible() {aIObjectID} was target[{targetThingID == aIObjectID}]"); }
